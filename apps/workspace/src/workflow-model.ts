...
export function buildWorkflow(task: Task, allEvents: Event[], attemptIndex?: number) {
  ...
  add("sys:planner", task.employee_id === "ai_assistant" ? "AI 管家 · 目标拆解与协作规划" : "任务调度 · 指定员工与依赖编排", "Supervisor / Planner",
    "选择专业员工、明确节点目标，校验依赖关系并形成协作计划。", ["sys:runtime"], [...category("planner."), ...category("plan.")],
    planned || plan.length ? (find("planner.fallback") ? "warning" : "done") : find("planner.started") ? "running" : "waiting",
    { output: planned?.payload.plan, note: planned ? ({ supervisor: "由 AI 管家生成计划", employee: "按指定员工编排", fallback: "规划失败，采用预置流程" }[text(planned.payload.source)] ?? "协作计划已记录") : "计划就绪后自动展开员工节点。" });
  ...
  for (const step of plan) {
    const matched = events.filter(e => e.type.startsWith("agent.node.") && e.payload.node_id === step.node_id);
    const end = matched.at(-1);
  ...
  if (feedback) {
    ...
    add("sys:recheck", "AI 管家 · 修订稿复核", "质量审核员", "复核修改是否解决初审问题，再决定归档或转人工。", ["sys:revision"], recheck,
